...
/** Ein Treffer pro Modelllinie – Reihenfolge des ersten Auftretens bleibt erhalten. */
export function dedupeMatchesByModelLine(matches = []) {
  ...
  for (const match of matches) {
    ...
    if (matchSortScore(match) > matchSortScore(existing)) {
      byLine.set(line, match);
    }
  }

  return order.map((line) => byLine.get(line));
}
...
export function buildModelLineGroups(allMatches = [], primaryMatches = [], options = {}) {
  ...
  const primaries = dedupeMatchesByModelLine(primaryMatches);
