...
// --- Golden turn ---
{
  ...
  assert.match(body, /Kaufpreis|Kondition/i);
  assert.equal(containsSellerCommandInMessage(body), false);
  assert.doesNotMatch(body, /Erstelle Herrn Garritano/i);
  assert.doesNotMatch(body, /\bRabatt\b/i);
  ...
  assert.match(review.title, /vorbereitet/i);
  assert.ok(review.actionSections.some((s) => s.kind === 'offer_and_message_review'));
...
// --- Ambiguous: Angebot oder Nachricht? ---
{
  const turn = runCleverSellerTurn({
    lead: {},
    sellerInput: 'Picanto GT-Line 17.000 €',
    leadsSnapshot: leads,
    scopeHint: 'dashboard',
  });
  assert.ok(
